  	...
  	if (searchfieldapplied) {
  		$.getJSON(
		    ...
		    function(jsonData,status) {
		    	var table = processCourseDataJSON(jsonData);
		    	modifySearchFieldsJSON(jsonData);
				$('#results').empty();
...
function processCourseDataJSON(jsonData) {
	var table = startTable()
	var tbody = $(document.createElement('tbody'));

	$.each(jsonData.courses, 
		function(i,course) {
			var tr = $(document.createElement('tr'));
			$(document.createElement('td')).append(course['catalog_number']).appendTo(tr);
			$(document.createElement('td')).append(course['course_group'] + '&#160;' + course['course_number']).appendTo(tr);
			$(document.createElement('td')).append(course['title']).appendTo(tr);
			$(document.createElement('td')).append(course['term']).appendTo(tr);
			$(document.createElement('td')).append(course['offered']).appendTo(tr);
			$(document.createElement('td')).append(course['course_type']).appendTo(tr);
			$(document.createElement('td')).append(course['course_level']).appendTo(tr);
			tr.appendTo(tbody);
		} // close function
	);  // close 'each' course
...
function modifySearchFieldsJSON(jsonData) {
	/* modify navigation */
	...
	for (var i = 0; i < fieldsconfig.length; i++) {		
		...
		$.each(jsonData.courses,
			function(i,course) {
				navpresentindata[key][course[key]] = 1;
				if (course[key] == 'Y') {
